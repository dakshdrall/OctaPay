...
export default function Explorer() {
  ...
  useEffect(() => {
    ...
    fetch(`${HORIZON}/accounts/${selected.address}/transactions?limit=10&order=desc`)
      ...
      .catch(() => { setError("Could not fetch from Horizon testnet"); setLoading(false); });
  }, [selected]);

  return (
    <div style={{ padding: "2rem", fontFamily: "var(--font-display)" }}>
      <h1 style={{ fontSize: "2rem", fontWeight: 800, color: "var(--color-cyan)", marginBottom: "1.5rem" }}>
        Explorer
      </h1>
      ...
      <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "var(--color-text-muted)", marginBottom: "1.5rem", wordBreak: "break-all" }}>
        {selected.address}
      </p>
      {loading && <p style={{ color: "var(--color-text-muted)" }}>Loading transactions...</p>}
      {error && <p style={{ color: "#f87171" }}>{error}</p>}
      {!loading && !error && txns.length === 0 && <p style={{ color: "var(--color-text-muted)" }}>No transactions found.</p>}
      {txns.map(tx => (
        <div key={tx.id} style={{ background: "var(--color-surface)", border: "1px solid var(--color-surface-2)", borderRadius: "12px", padding: "1rem 1.25rem", marginBottom: "0.75rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem" }}>
            <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "var(--color-cyan)" }}>{tx.id.slice(0, 16)}...</span>
            <span style={{ fontSize: "0.75rem", color: "var(--color-text-muted)" }}>{new Date(tx.created_at).toLocaleString()}</span>
          </div>
          <div style={{ marginTop: "0.5rem", fontSize: "0.85rem", color: "var(--color-text-muted)" }}>
            Fee: {tx.fee_charged} stroops · Ledger: {tx.ledger}
          </div>
          <a href={`https://stellar.expert/explorer/testnet/tx/${tx.id}`} target="_blank" rel="noopener noreferrer"
            style={{ fontSize: "0.75rem", color: "var(--color-cyan)", textDecoration: "none", marginTop: "0.25rem", display: "inline-block" }}>
